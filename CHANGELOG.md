...
- Add the WhatsApp suppression methods under `whatsapp.suppressions`: read the addresses your workspace will not message, suppress one for a single business account or for the whole workspace, and end a suppression you added. A record is one period of suppression rather than a current state, so the list returns the periods in force while a read by ID also resolves one that has ended, with when and how it ended. Only a suppression you added yourself can be ended; a recipient's own opt-out is theirs to reverse.
- Registering an SMS sender for a country whose short codes Bird files now returns `422` up front instead of creating a pending registration that could never complete; read the country's requirement row for progress.
...
- Read how far a group send got from `recipient_count`, `delivered_count` and `read_count` on the message, and which participant each confirmation came from on the `recipient` of its delivered and read events.
- Filter messages by `group_id` to list one group's conversation, inbound and outbound.
...
- Verification webhook `status`, WhatsApp suppression webhook `reason`, and template-button `otp_type` now accept future string values.
- Open-enum model fields now enforce their declared string constraints.
- WhatsApp interactive-message models now restrict `type` to the values allowed for each message, button, and header.
...
- **Breaking:** `Verification.expires_at`, `Domain.created_at`, and `Domain.updated_at` now reject empty strings; callers constructing these models must provide non-empty values.
...
- Fix synchronous and asynchronous list iteration after an `ending_before` cursor. Subsequent pages clear the backward cursor when advancing with `starting_after`.
...
- Add `client.whatsapp.templates` methods to list and get WhatsApp templates, their versions, and each version's languages.
...
- Fields with custom examples now enforce their declared validation rules, including template `slug` and `language` values and WhatsApp `in_reply_to_message_id`.
...
- **Breaking:** `rejection_reason` on a voice call record gains `number_ownership_not_verified`, and a client built before this release raises on parsing a call refused for it. Upgrade before reading call records, or read `rejection_reason` as a plain string.
